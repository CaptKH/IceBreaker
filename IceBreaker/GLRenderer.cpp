...
int GLRenderer::Initialize(void) {
	...
	GenerateVertexBuffer(vertexBufferObj, vertexData);

	#pragma region Fragment Data
	...
	GenerateFragmentBuffer(fragmentBufferObj, fragmentData);
...
/* DrawCube */
void GLRenderer::DrawCube(Matrix4& model, Matrix4& view, RenderMode r) {
...
void GLRenderer::GenerateFragmentBuffer(GLuint fbo, GLfloat data[]) {
	// Create spot for VBO on GPU
	glGenBuffers(1, &fbo);
	// Bind it
